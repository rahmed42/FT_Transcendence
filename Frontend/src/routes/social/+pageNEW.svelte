<script lang="ts">
	...
	onMount(async () => {
		...
		socket.on('friend-request', (data: { login: string }) => {
			if (data.login == $user.login || data.login == '') refreshData();
		});
	});
...
<section>
	<button id="send_friend_request" class="button-styled" on:click={openFriendRequestModal}>Send a friend request</button>
	{#if friendRequestModalOpen}
		<div class="modal">
			<div class="modal-content">
				<h3> Friend request</h3>
				<input type="text" bind:value={requesteeLoginModal} placeholder="Friend's user login" /> <br /> <br />
	...
	<div class="friends-container">
		{#each friends as friend (friend.id)}
			<div class="friend-card">
				...
				<a href={`/profile/info/?login=${friend.friend.login}`} class="button-style-light"
					>View Profile</a
				>
			</div>
		{/each}
